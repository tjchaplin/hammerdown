...
HammerDown.prototype.text = function(text) {
	...
	if(self.writerState.isInBlockQuote()){
		self.prefixWithBlockQuote(text);
		return self;
	}

	if(self.writerState.isInBlockCode()){
		self.prefixWithBlockCode(text);
		return self;
	}
	...
	var containsLettersOrNumbers = formattedText.match(/\w|\d| |\t/);
...
HammerDown.prototype.prefixData = function(data,prefixValue,prefixDepth){
	...
	this.documentStream.appendWithPrefix(prefix,data);
	return data;
};
...
HammerDown.prototype.appendRawData = function(data){
	if(this.documentStream.atDocumentStart)
		data = data.replace(/^[\n]+/,'');

	if(this.documentStream.atNewline)
		data = data.replace(/^[\n]{1}/,'');

	if(this.writerState.isInBlockQuote()){
		this.prefixWithBlockQuote(data);
		return this;
	}

	if(this.writerState.isInBlockCode()){
		this.prefixWithBlockCode(data);
		return this;
	}

	this.documentStream.appendRaw(data);
};
HammerDown.prototype.appendFormattedData = function(data){
	if(this.documentStream.atDocumentStart)
		data = data.replace(/^[\n]+/,'');

	if(this.documentStream.atParagraphEnd)
		data = data.replace(/^[\n]{2}/,'');
	...
	this.documentStream.appendFormatted(data);
};
...
HammerDown.prototype.headerOpen = function(level){
	...
	this.appendFormattedData(headerOutput);
	return this;
};
...
HammerDown.prototype.listOpen = function(){
	...
	this.appendFormattedData(listOutput);
	return this;
};
...
HammerDown.prototype.listItemOpen = function(){
  ...
  this.appendFormattedData(listItem);
  return this;
};
...
HammerDown.prototype.linkOpen = function(linkDefinition){
	...
	if(linkDefinition && (/^mailto:/).test(linkDefinition.href))
		linkOpenText = '<';

	this.documentStream.appendFormatted(linkOpenText);
	this.writerState.saveLinkDefinition(linkDefinition);
	return this;
};
...
HammerDown.prototype.linkClose = function(){
	...
	if((/^mailto:/).test(linkDefinition.href)){
		this.documentStream.appendFormatted('>');
		this.writerState.previousState();
		return this;
	}
